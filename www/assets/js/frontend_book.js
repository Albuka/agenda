...
var FrontendBook = {
    ...
    getAvailableHours: function(selDate) {
        ...
        var ar = jQuery.post(ajaxurl, postData, function(response) {
            ///////////////////////////////////////////////////////////////
            ...
            if (response.length > 0) {
                var currColumn = 1;
                $('#available-hours').html('<div style="width:100%; float:left;"></div>');

                $.each(response, function(index, availableHour) {
                    // if ((currColumn * 12) < (index + 1)) {  // deixei em uma coluna única
                        // currColumn++;
                        // $('#available-hours').append('<div style="width:100%; float:left;"></div>');
                    // }
        ...
        $('#appointment-details').html(
            ...
            '<p>' 
        		+ '<strong class="text-info">' 
                    ...
                    + '<br>' + serviceCurrency + ' ' + servicePrice
    			+ '</strong>' + 
            '</p>'
        );
        ...
        $.each(GlobalVariables.availableServices, function(index, service) {
            if (service.id == serviceId) { // Just found the service.
                ...
                if (service.description != '' && service.description != null) {
                    html += '<br><em>' + service.description + '</em><br>';
                }
                ...
                if (service.price != '' && service.price != null) {
                    html += ' - ' + EALang['price'] + ': ' + service.currency + ' ' + service.price  ;
                }   
